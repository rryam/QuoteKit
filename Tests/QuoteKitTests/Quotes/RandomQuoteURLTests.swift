...
final class RandomQuoteURLTests: XCTestCase {
  ...
  func testURLForRandomQuote() {
    ...
    try XCTAssertEqual(url, host.expectedURL(with: "quotes/random"))
  }

  func testURLForMinimumLengthQuote() {
    let url = QuotableEndpoint(.randomQuote, queryItems: [.minLength(10)]).url
    try XCTAssertEqual(url, host.expectedURL(with: "quotes/random?minLength=10"))
  }

  func testURLForMaximumLengthQuote() {
    let url = QuotableEndpoint(.randomQuote, queryItems: [.maxLength(100)]).url
    try XCTAssertEqual(url, host.expectedURL(with: "quotes/random?maxLength=100"))
  }
  ...
  func testURLForMinimumAndMaximumLengthQuote() {
    ...
    try XCTAssertEqual(url, host.expectedURL(with: "quotes/random?minLength=10&maxLength=100"))
  }
  ...
  func testURLWithEitherOfTheProvidedTagsParameter() {
    let url = QuotableEndpoint(.randomQuote, queryItems: [.tags(["love", "happiness"], .either)])
      .url
    try XCTAssertEqual(url, host.expectedURL(with: "quotes/random?tags=love|happiness"))
  }
  ...
  func testURLWithAllOfTheProvidedTagsParameter() {
    ...
    try XCTAssertEqual(url, host.expectedURL(with: "quotes/random?tags=technology,famous-quotes"))
  }

  func testURLForSpecificAuthor() {
    let url = QuotableEndpoint(.randomQuote, queryItems: [.authors(["albert-einstein"])]).url
    try XCTAssertEqual(url, host.expectedURL(with: "quotes/random?author=albert-einstein"))
  }
  ...
  func testURLForManyAuthors() {
    ...
    try XCTAssertEqual(url, host.expectedURL(with: "quotes/random?author=albert-einstein|ed-cunningham"))
  }
}
